...
const def = [
/* NOUN Rules to build gender neutrals and plurals and declinations
- enter NON-animal/person-like nouns with article
- article for female nouns is optional
'das Haus'
'die Erde' or 'Erde'

- enter animal/person-like nouns without article as male, marked with *
'Pilot*' [> die Pilot*in]
'Kunde*' [> die Kund*in]

- mark anglicisms with ^
'der Link^',
'Coach*^'

- mark compounds with | (except they have a -)
'der Grab|stein'
'Geordnete-Sammlung-Seite'
...
*/
  ...
  'Ignore,Ignoranz',
  'Join,Beitritt,m',
  'Leave,Austritt,m', //Verlassen
 	'Like,Gefallen,n',
  'Offer,Angebot,n', //AnBieten
  'Invite,Einladung',
  'Reject,Ablehnung',
  'TentativeReject,Vorläufige Ablehnung',
  'Remove,Entfernung',
	'Undo,Rückgängig-Machen,n',
  ...
  'Mention,Erwähnung',
  'Profile,Profil,n',
  'Tombstone,Grab|stein,m'
].reduce((o,_def) => {
	// Ausnahme Herz
	const [key, noun, gender = 'f'] = _def.split(',');
  const declinedNoun = declineNoun(noun, gender);
  for (let k in declinedNoun) {
    o[k === 'SUB' ? key : `${key}_${k}`] = declinedNoun[k];
  }
  return o
}, {});
console.log(def);
